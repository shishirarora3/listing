...
const useVerticalPosition = (desiredPositions, pos) => {
  const [positionDirection, setPositionDirection] = useState("bottom");
  useEffect(() => {
    const canBePositionTop = pos.containerTop - pos.popoverHeight > 0;
    const canBePositionBottom = pos.containerTop + pos.containerHeight + pos.popoverHeight < pos.windowHeight; // returns the position name if the position can be set
